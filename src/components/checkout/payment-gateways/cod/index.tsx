...
export default function CodPaymentGateway(props: PaymentGatewaysProps) {
    const { cart } = useCart();
    ...
    const dAmount = parseFloat(`${total}`);

    if (dAmount > 300) {
        amount = 300;
    }

    const description: string = `Pay ${amount} online and ${dAmount - amount} with cash on delivery.`;
