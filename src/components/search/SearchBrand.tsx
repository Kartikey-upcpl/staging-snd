...
import { CustomMenuBrand } from './CustomBrand';
...
interface SearchBrandProps {
    slug: string;
    searchClient?: any;
    brandsPromise: Promise<Brand[] | ApiError>;
}
...
export default function SearchBrand({
    ...
    const listBrand = use(brandsPromise);
    ...
    const tag = brands.find(t => t.slug === currentSlug);
                ...
                routing={{
                    router: {
                        ...
                        createURL({ qsModule, routeState, location }) {
                            ...
                            const tagPath = menu?.["brands.slug"] ?? '';
                            ...
                            return `${baseUrl}brand/${tagPath}${queryString}`;
                        },
                        parseURL({ qsModule, location }) {
                            ...
                            const slugBrand = location.pathname.replace('/brand/', '');
                            ...
                            if (currentSlug !== slugBrand) {
                                setCurrentSlug(slugBrand);
                            }
                            return {
                                [indexName]: {
                                    page,
...
function Filter({ brands } : { brands: Brand[] }) {
    return (
        <>
            <Accordion label="Brands">
                <CustomMenuBrand attribute={"brands.slug"} brands={brands} />
            </Accordion>
